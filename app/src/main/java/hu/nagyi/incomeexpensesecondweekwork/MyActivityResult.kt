...
class MyActivityResult<Input, Result> private constructor(
    ...
    /**
     * Launch activity, same as [ActivityResultLauncher.launch]
     * except that it allows a callback executed after receiving a result from the target activity.
     */
    /**
    ...
    companion object {
         ...
         * activity result callback like the default approach.
         * You can still customise callback using [.launch].
         */
        fun <Input, Result> registerForActivityResult(
            caller: ActivityResultCaller,
            contract: ActivityResultContract<Input, Result>,
            onActivityResult: OnActivityResult<Result>?
        ): MyActivityResult<Input, Result> {
            return MyActivityResult(caller, contract, onActivityResult)
        }

        /**
         * Same as [.registerForActivityResult]
         * except the last argument is set to `null`.
         */
        fun <Input, Result> registerForActivityResult(
            caller: ActivityResultCaller,
            contract: ActivityResultContract<Input, Result>
        ...
        fun registerActivityForResult(
            caller: ActivityResultCaller
        ): MyActivityResult<Intent, ActivityResult> {
